...
export function DefaultNode({
  ...
  const [led1, setLed1] = useState<boolean>(true)
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [ledPower, setLedPower] = useState<boolean>(true)
  interface Signal {
    type: string;
    signal?: string;
    voltage?: number;
  ...
  function spi(signal: string, index: number): Signal {
    return { type: 'spi', signal: `${signal}_${index}` };
  }

  function usart(signal: string, index: number): Signal {
    return { type: 'usart', signal: `${signal}_${index}` };
  }
  ...
  return (
    <>
      ...
      <div style={{ position: 'relative' }}>
        {/* Render handles for each pin - both source and target */}
        {pinInfo.map((pin) => {
          const handleStyle = {
        ...
        >
          ...
          <text x="30" y="59" fill="#cecccb" fontFamily="sans-serif" fontSize="15px">ESP32</text>
