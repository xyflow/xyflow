...
const SmallSoundSensorNode = ({ id }: NodeProps<BuiltInNode>) => {
  const [ledPower] = useState(false);
  const [ledSignal] = useState(false);

  const pinInfo = [
    { name: 'AOUT', y: 11, x: 0, number: 1, signals: [] },
    { name: 'GND', y: 20.5, x: 0, number: 2, signals: [{ type: 'power', signal: 'GND' }] },
    { name: 'VCC', y: 30.5, x: 0, number: 3, signals: [{ type: 'power', signal: 'VCC' }] },
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          <pattern id={`pin-pattern-${id}`} height="3.6" width="14" patternUnits="userSpaceOnUse">
            <path
        ...
        <use xlinkHref={`#header-pin-${id}`} x="26.6" y="12.4" />
        <use xlinkHref={`#header-pin-${id}`} x="26.6" y="22.1" />
        <use xlinkHref={`#header-pin-${id}`} x="26.6" y="31.9" />
        <use xlinkHref={`#header-pin-${id}`} x="26.6" y="41.6" />
        ...
        <filter id={`ledFilter-${id}`} x="-0.8" y="-0.8" height="5.2" width="5.8">
          <feGaussianBlur stdDeviation="2" />
        </filter>
        {ledPower && <circle cx="56" cy="4" r="7" fill="#80ff80" filter={`url(#ledFilter-${id})`} />}
        ...
        {ledSignal && (
          <circle cx="56" cy="47" r="7" fill="#80ff80" filter={`url(#ledFilter-${id})`} />
        )}
...
export default memo(SmallSoundSensorNode);
