...
/**
 * Destructure the reactive state from {@link useVueFlowStore} into refs without losing reactivity (Pinia's
 ...
 *
 * ```ts
 * const { nodes, transform } = storeToRefs(useVueFlowStore()) // Ref<Node[]>, Ref<Transform>
 * ```
 *
 * Reference-type fields (the `nodeLookup`/`parentLookup`/`edgeLookup`/`connectionLookup` Maps) stay
 * reactive when destructured straight off `useVueFlowStore()` — you only need this for the value-type fields.
 *
 * @public
 */
