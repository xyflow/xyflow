...
import { OnNodesChange, Panel, ReactFlow, ReactFlowProps, useReactFlow, Node } from '@xyflow/react';
...
describe('<ReactFlow />: onNodesChange', () => {
  ...
  it('receive position change', () => {
    ...
    const expectedChanges = [{ id: '1', type: 'position', dragging: false, position: endPosition }];
...
function Comp(props: CompProps) {
  ...
  const onNodesChange: OnNodesChange<Node> = useCallback((changes) => {
    props.onNodesChange(changes.toArray());
    setNodes((nds) => changes.applyTo(nds));
  }, []);
