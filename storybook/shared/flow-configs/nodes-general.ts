...
export const nodesGeneralReactConfig = {
  flowProps: {
    ...nodesGeneralBaseProps,
    deleteKeyCode: 'd',
...
export const nodesGeneralSvelteConfig = {
  flowProps: {
    ...
    nodes: nodesGeneralNodes.map((node) => {
      const { className, style, ...rest } = node as SharedNode & {
        className?: string;
