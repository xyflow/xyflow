import type { FlowFramework, SharedFlowConfig, SharedNode } from '../types';
...
export const paneNonDefaultsConfig = {
  flowProps: {
    panOnScroll: true,
...
export const edgesGeneralConfig = {
  flowProps: {
    fitView: true,
    multiSelectionKeyCode: 's',
    deleteKeyCode: 'd',
    nodes: edgesGeneralNodes,
    edges: edgesGeneralEdges,
  },
} satisfies SharedFlowConfig;
...
export const nodesGeneralConfig = {
  flowProps: {
    fitView: true,
    nodeDragThreshold: 0,
    autoPanOnConnect: false,
...
function adaptConfig(config: SharedFlowConfig, framework: FlowFramework): SharedFlowConfig {
  if (framework === 'react') return config;
  const { nodes, edges, ...props } = config.flowProps ?? {};
  if (framework === 'svelte') {
    for (const [from, to] of [
      ['deleteKeyCode', 'deleteKey'],
    ...
    if (props.multiSelectionKey) props.multiSelectionKey = ['Meta', props.multiSelectionKey];
    if (props.initialViewport) props.fitView = false;
    if (config === nodesGeneralConfig) {
      props.selectionOnDrag = true;
      props.panOnDrag = false;
    }
  }
  const adaptElement = <T extends Record<string, unknown>>(element: T) => {
    const { className, style } = element;
    const adapted = { ...element };
    delete adapted.className;
    const convertedStyle =
      framework === 'svelte' && style && typeof style === 'object'
        ? Object.entries(style)
            .map(([key, value]) => key.replace(/[A-Z]/g, (letter) => '-' + letter.toLowerCase()) + ': ' + value + ';')
            .join(' ')
        : style;
    return { ...adapted, ...(className ? { class: className } : {}), ...(style ? { style: convertedStyle } : {}) };
  };
  return { flowProps: { ...props, nodes: nodes?.map(adaptElement), edges: edges?.map(adaptElement) } };
}

export function createTestConfigs(framework: FlowFramework) {
  return {
    paneGeneral: adaptConfig(paneGeneralConfig, framework),
    paneNonDefaults: adaptConfig(paneNonDefaultsConfig, framework),
    nodesGeneral: adaptConfig(nodesGeneralConfig, framework),
    edgesGeneral: adaptConfig(edgesGeneralConfig, framework),
    colorMode: adaptConfig(colorModeConfig, framework),
  };
}
